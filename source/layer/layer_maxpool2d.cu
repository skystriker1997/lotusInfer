...
namespace lotus {
    ...
    void Maxpool2dLayer::Forward() {
        ...
        for(int i=0; i<batch_size; ++i) {
            ...
            Maxpool2d<<<MakeMP2dGrid(x_c, y_h, y_w), MakeMP2dBlock(), 0, pool.Stream()>>>(x.Data(), 
                                                                                           y.Data(),
                                                                                           kernel_h_, kernel_w_,
    ...
    std::shared_ptr<Maxpool2dLayer> MakeMaxpool2dLayer(pnnx::Operator *opt, const std::map<std::string, std::shared_ptr<Operand>>& operands) {
        CHECK(opt->inputs.size()==1) << "maxpool2d layer is supposed to accept 1 input";  
        CHECK(opt->outputs.size()==1) << "maxpool2d layer is supposed to generate 1 output";

        auto dilation = opt->params.find("dilation");
        CHECK(dilation != opt->params.end()) << "maxpool2d layer missing parameter 'dilation'";
        CHECK(dilation->second.ai[0]==1 && dilation->second.ai[1]==1) << "maxpool2d layer supports only kernel dilation with value of 1";

        auto padding = opt->params.find("padding");
        CHECK(padding != opt->params.end()) << "maxpool2d layer missing parameter 'padding'";

        auto stride = opt->params.find("stride");
        CHECK(stride != opt->params.end()) << "maxpool2d layer missing parameter 'stride'";

        auto kernel_size = opt->params.find("kernel_size");
        CHECK(kernel_size != opt->params.end()) << "maxpool2d layer missing parameter 'kernel_size'";

        auto ceil_mode = opt->params.find("ceil_mode");
        CHECK(ceil_mode != opt->params.end()) << "maxpool2d layer missing parameter 'ceil_mode'";
        ...
        CHECK(return_indices != opt->params.end()) << "maxpool2d layer missing parameter 'return_indices'";
        ...
        CHECK(input != operands.end()) << "maxpool2d layer missing input operand";
        auto output = operands.find(outputs_name[0]);
        CHECK(output != operands.end()) << "maxpool2d layer missing output operand";
